/* Example derived from libwmf bug: realloc result ignored and original used */
#include <cstdlib>
#include <cstdio>

typedef struct { int count; int max; void *list; } gdClip;
...
void gdClipSetAdd(gdImage *im, gdClipRectangle *rect) {
    ...
    if (im->clip->count == im->clip->max) {
        ...
        if (more == 0) return;
        im->clip->max += 8;
        /* missing: im->clip->list = more; */
    }
    /* may access freed memory */
    ((gdClipRectangle*)im->clip->list)[im->clip->count] = *rect;
    im->clip->count++;
}
